predict.geno.values.family.assisted <- function(full_geno, pheno_train, geno_train, pheno_val, val_pedigree, geno_val, prop, cycles) { # function hardcorded for Traits
    ...
    for(r in 1:cycles){
        ...
        selected_validation_pheno <- sample.validation.by.pedigree(pheno_val, val_pedigree, num)
        selected_validation_pheno <- selected_validation_pheno %>% dplyr::select(-Pedigree)
        ...
        # validation stuffs
        new_val_geno <- geno_val %>% anti_join(selected_validation_id,  by = "ID")
        take_out_val_geno <- selected_validation_id %>% inner_join(geno_val, by = "ID")
        val_id_to_add_pheno <- selected_validation_pheno
